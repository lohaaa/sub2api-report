...
> 项目当前处于 0.7.0 持久化计划任务阶段：报告支持滚动 7/30 日、上一完整自然周、上一完整自然月和手工自定义区间，生成 schema v4 不可变快照与动态 CSV；计划窗口保存在 SQLite 并在 Quartz 任务入队时冻结。规范化执行记录、任务级重试、逐渠道状态与失败补发已经实现。在线升级与正式发布尚未实现。
