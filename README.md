...
> 项目当前处于 0.7.0 持久化计划任务阶段：报告支持滚动 7/30 日、上一自然周、上一自然月和手工自定义区间，生成 schema v4 不可变快照与动态 CSV；计划窗口保存在 SQLite 并在 Quartz 任务入队时冻结。规范化执行记录、任务级重试、逐渠道状态与失败补发已经实现。GitHub Release 离线 bundle 工作流已建立，但应用内在线升级仍未实现；`0.x` 仅作为内部里程碑，首次公开版本将是 `v1.0.0`。
