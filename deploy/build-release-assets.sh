...
contract_file="$repo_root/deploy/upgrade-contract.json"
...
for command_name in awk docker dotnet gzip install jq openssl sed sha256sum sort stat tar; do
...
jq -e '
  ...
  and .update.databaseBackupRequired == true
  and .update.automaticRollbackRequired == true
' "$contract_file" >/dev/null || {
  echo "deploy/upgrade-contract.json is invalid for the App-only release." >&2
  exit 2
}
contract_version=$(jq -r '.deploymentContractVersion' "$contract_file")
...
install -m 0644 "$contract_file" "$output_dir/upgrade-contract.json"
...
actual_os=$(docker image inspect "$app_version_tag" --format '{{.Os}}')
actual_arch=$(docker image inspect "$app_version_tag" --format '{{.Architecture}}')
actual_version=$(docker image inspect "$app_version_tag" --format '{{index .Config.Labels "org.opencontainers.image.version"}}')
actual_role=$(docker image inspect "$app_version_tag" --format '{{index .Config.Labels "io.sub2api-report.role"}}')
actual_contract=$(docker image inspect "$app_version_tag" --format '{{index .Config.Labels "io.sub2api-report.contract"}}')
[[ $actual_os == linux && $actual_arch == amd64 ]] || {
  echo "$app_version_tag has unexpected platform $actual_os/$actual_arch" >&2
  exit 1
}
[[ $actual_version == "$version" && $actual_role == app && $actual_contract == "$contract_version" ]] || {
  echo "$app_version_tag has unexpected release labels" >&2
  exit 1
}
...
inspect_saved_image_archive() {
  ...
  [[ $config_digest == "sha256:$config_hex" ]] || return 1
  ...
  [[ $target_digest =~ ^sha256:[a-f0-9]{64}$ ]] || return 1
...
app_digests=$(inspect_saved_image_archive "$output_dir/$app_asset" "$app_version_tag") || {
  echo "App image archive metadata validation failed." >&2
  exit 1
}
IFS=$'\t' read -r app_image_id app_target_digest <<<"$app_digests"
...
[[ -n $target_migration ]] || {
  echo "No EF Core migration was found for the release manifest." >&2
  exit 1
}
...
install -m 0644 "$output_dir/upgrade-contract.json" "$bundle_dir/upgrade-contract.json"
...
(
  cd "$output_dir"
  sha256sum "$app_asset" "$bundle_asset" "$server_asset" \
    release-manifest.json release-manifest.sig upgrade-contract.json update-public-key.pem \
