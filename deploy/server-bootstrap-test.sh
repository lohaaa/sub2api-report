...
printf '%s  sub2api-report-server-v1.0.2-linux-amd64.tar.gz\n' \
...
case "$url" in
  */releases/latest)
    printf 'https://github.com/lohaaa/sub2api-report/releases/tag/v1.0.2'
    ;;
  */sub2api-report-server-v1.0.2-linux-amd64.tar.gz)
...
grep -Eq '^bash /tmp/sub2api-report-server-bootstrap\..*/bundle/server-install\.sh$' \
  "$test_root/sudo.log"
test "$(wc -l < "$test_root/curl.log")" -eq 3
grep -q '^Release resolved: v1.0.2$' "$test_root/output.log"
