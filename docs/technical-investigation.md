...
`total_cost` 是标准计费金额；`total_actual_cost` 是应用分组等倍率后的实际用户计费金额。上游用量页面使用美元符号展示这两项费用，因此报告统一以 USD 标注；月报建议同时保留两者，以 `total_actual_cost` 作为默认费用列。
...
相比登录后保存 JWT，这种方式没有 access token 刷新和 2FA 登录问题。权限很高，因此必须通过管理页面录入、加密保存到 SQLite，并限制报表容器和网络访问范围；日志和读取 API 不能返回明文。
...
0.3.0 使用管理接口的 `items`、`total`、`page`、`page_size` 和 `pages` 完成全量分页。上游 DTO 只声明 `id`、`name`、`status`、`group_id` 和 `last_used_at` 等快照字段，不声明响应中的完整 `key`，未知字段由 JSON 解析器跳过。

同步在完整获取全部分页后才开启单次 SQLite 更新：已有 ID 更新名称和状态，成功响应中缺失的旧 ID 标记为退休。任何分页鉴权、兼容性、限流、服务错误、超时或 JSON 错误都会保留上一次完整清单。

浏览器契约把上游 `int64` ID 表示为十进制字符串，避免 JavaScript `number` 超过 `2^53-1` 后失真；SQLite 仍使用整数列和唯一索引。
...
0.4.0 调用管理 stats 接口时同时传目标 `user_id`、`api_key_id`、可选 `group_id`、闭合自然日起止日、IANA 时区和 `nocache=true`。上游 JSON 数值直接解析为 `decimal`/`long`；浏览器 API 把 64-bit 请求数和 Token 表示为十进制字符串。

报告引擎对每个 Key 和已解析窗口调用 stats；同一 Key 下解析成完全相同日期边界的窗口只采集一次。每个请求最多尝试 3 次，报告级并发来自 SQLite 设置且限制在 1 到 10。任一区间失败仍保存 `Partial` 快照，并在诊断中记录窗口 Key 与半开日期边界。

## 3. 统计口径

### 3.1 推荐窗口
月报在每月 1 日运行，统计运行日之前的完整自然日，不包含 1 日当天的部分数据。默认同时生成四个窗口；手工报告还可以增加一个自定义闭合日期区间。

例如报告日为 2026-08-01：

| 窗口 | 开始日期 | 展示结束日期 | 天数 |
| --- | --- | --- | ---: |
| 滚动 7 日 | 2026-07-25 | 2026-07-31 | 7 |
| 滚动 30 日 | 2026-07-02 | 2026-07-31 | 30 |
| 上一自然周（周一开始） | 2026-07-20 | 2026-07-26 | 7 |
| 上一自然月 | 2026-07-01 | 2026-07-31 | 31 |

领域模型和 canonical snapshot 使用半开边界 `[StartDate, EndDateExclusive)`；页面、XLSX 和上游请求适配层才显示或传递闭合结束日。滚动窗口允许 1 到 90 日，自定义区间最多 92 日且不允许进入循环计划。
...
> 0.6.0 起本节取代旧的“Key 到人员映射”设计：报告统计主体是 Sub2API 用户 →
> API Key，不再维护人员档案。稳定标识是 `user_id + api_key_id`；Key 名称在
> 每次报告生成前自动刷新并保存快照，只用于展示。历史快照不可变。

聚合规则：

- 报告按用户分组展示每个 Key 在全部已选窗口中的用量，并提供用户小计与全部总计；
- 生成前自动刷新用户与 Key，刷新失败终止本次报告并记录到 `ReportGenerationRuns`；
- 已删除/轮换的 Key 在本地保留 `RetiredAt`，保证历史窗口完整；
- 报表中不得展示完整 Key 明文。

上游当前删除 Key 的实现是软删除并替换凭证明文，因此历史 ID 可以继续作为统计标识。
...
- Sub2API 用户
- Key 数量
- 每个窗口的请求数、输入 Token、输出 Token、缓存 Token、总 Token、实际费用和按实际天数计算的日均费用
- 窗口 Key、窗口名称、类型、开始日期、展示结束日期和实际天数
- 最后使用时间

单位统一写入字段或列标题，例如“请求数（次）”“Token 数（个）”“实际费用（USD）”；页面、通知表格和 XLSX 的数据值保持纯数字，不添加货币符号或单位前后缀，便于复制和电子表格统计。
...
- 正文使用 HTML 汇总表。
- 附件使用多工作表 XLSX 工作簿，含报告概览、Key 明细、用户汇总、条件采集异常和数据说明。
- 主题示例：`[Codex 用量月报] 2026-07-02 至 2026-07-31`。
- 凭证通过管理页面录入并加密保存到 SQLite，更新后下一次发送动态生效。
...
- 自定义机器人 webhook 没有文件附件消息类型；系统改为在摘要中提供可撤销的限时 XLSX 下载链接。

### 5.3 飞书

- 使用群自定义机器人 webhook。
- 启用签名校验，签名和 webhook secret 通过管理页面录入并加密保存。
- 使用 `post` 富文本消息；内容过长时分片，并在每片标注 `1/N`。
- 校验 HTTP 状态码和响应 JSON 错误码。
- 官方文档给出的自定义机器人限制是每个租户、每个机器人 100 次/分钟且 5 次/秒，请求体不超过 20 KB。
- 群自定义机器人不能调用应用机器人的文件上传 API；系统在富文本摘要中提供可撤销的限时 XLSX 下载链接，不扩大为应用机器人鉴权方案。
...
- 外部访问基址、有效期和下载次数上限存入 SQLite 并动态生效；有效期允许 1 小时到 30 天，次数可以限制为 1 到 10000 或不限制。
- 每条钉钉/飞书投递冻结当时的授权策略；消息发送成功后才开始计时，失败重试和进程恢复复用同一令牌。
- SQLite 保存 SHA-256 令牌哈希和 Data Protection 密文，不保存明文；管理员可在报告投递记录中提前撤销。
- 匿名下载端点按令牌校验到期、撤销和次数上限，成功下载计数；响应使用 `private, no-store`、`Referrer-Policy: no-referrer` 和独立 IP 限流。
- 链接属于 bearer credential，群成员可在有效期内转发；系统只有单管理员，无法把链接绑定到具体群成员身份。
...
| 人员过多导致消息过长 | webhook 发送失败或难阅读 | 邮件附完整 XLSX 工作簿；IM 消息分片或只发摘要 |
