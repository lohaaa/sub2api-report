...
30 日窗口按 7 日起点和归属起止日切成原子区间，因此同一 Key 在窗口内换人时不会全部归给当前人员。每个区间最多尝试 3 次，报告级并发来自 SQLite 设置且限制在 1 到 10。任一区间失败仍保存 `Partial` 快照，并明确列出失败范围；全员总计包含成功采集的未归属用量，人员总计只包含唯一有效归属。
...
- 报告按用户分组展示每个 Key 的 7/30 日用量，并提供用户小计与全部总计；
