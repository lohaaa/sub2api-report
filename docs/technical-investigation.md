...
领域模型和 canonical snapshot 使用半开边界 `[StartDate, EndDateExclusive)`；页面、CSV 和上游请求适配层才显示或传递闭合结束日。滚动窗口允许 1 到 90 日，自定义区间最多 92 日且不允许进入循环计划。
...
单位统一写入字段或列标题，例如“请求数（次）”“Token 数（个）”“实际费用（USD）”；页面、通知表格和 CSV 的数据值保持纯数字，不添加货币符号或单位前后缀，便于复制和电子表格统计。
...
- 自定义机器人 webhook 没有文件附件消息类型；系统改为在摘要中提供可撤销的限时 CSV 下载链接。
...
- 群自定义机器人不能调用应用机器人的文件上传 API；系统在富文本摘要中提供可撤销的限时 CSV 下载链接，不扩大为应用机器人鉴权方案。
